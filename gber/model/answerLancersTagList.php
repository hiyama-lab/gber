...
//重複確認
$result2
    = mysql_query("SELECT socialactivityid FROM lancers_tag_list WHERE infoid = '"
    . $post['infoid'] . "' and username = '" . $post['username'] . "'")
...
$result3
    = mysql_query("SELECT socialactivityid FROM lancers_tag_list WHERE infoid = '"
    . $post['infoid'] . "'") or die ("Query error: " . mysql_error());
if (mysql_num_rows($result3) > 2) {
    exit;
}
...
echo $_GET['jsoncallback'] . '({"last_id":' . $last_id . '});';
